...
function drawBoard() {
  ...
  const boardToDraw = playerColor === 'white' ? board : [...board].reverse();
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      const realRow = playerColor === 'white' ? row : 7 - row;
      const piece = board[realRow][col];
...
function getMousePos(evt) {
  ...
  return playerColor === 'white' ? {x, y} : {x, y: 7 - y};
}
...
canvas.addEventListener('click', e => {
  ...
  if (selected) {
    ...
    if (sx !== y || sy !== x) {
      if (isValidMove(sx, sy, y, x)) {
        board[y][x] = board[sx][sy];
        board[sx][sy] = "";
        selected = null;
        statusText.textContent = "Bot thinking...";
        drawBoard();
        if (gameMode === 'bot') setTimeout(botMove, 1000);
        return;
      }
    }
    selected = null;
  } else if (clicked && clicked.startsWith(playerColor[0])) {
    selected = [y, x];
  }
  drawBoard();
});
...
function botMove() {
  for (let y = 1; y < 7; y++) {
    for (let x = 0; x < 8; x++) {
      ...
      if (piece && piece === 'b_p' && !board[y + 1][x]) {
        board[y + 1][x] = 'b_p';
        board[y][x] = "";
        drawBoard();
